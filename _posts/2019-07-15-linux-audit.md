...
因为Linux审计守护进程可以提供有价值的审计数据，所以Lynis将检查框架是否存在。如果不可用，它将建议您安装。此外，Lynis将执行几个测试来确定日志文件、可用规则等。

对于正确的入侵检测，与入侵检测系统(IDS)集成是在事件发生时发现事件并采取适当行动的关键。
...
审计守护进程有更多的可能性。其他例子将在本博客的后续文章中单独列出。如果您认真考虑审核Linux平台，那么Linux审核框架绝对是您的好朋友!