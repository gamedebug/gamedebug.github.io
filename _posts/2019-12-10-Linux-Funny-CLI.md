...
字面意思很清楚，奶牛说。这个命令能让命令行以一头奶牛（不限于奶牛）的字符卡通形象说话的方式显示任何我们想要显示的内容，先来读一读命令的手册：
...
这个清单里列出的所有以.cow为后缀的文件就是当前这个版本所支持的字符卡通形象，我们可以在输入cowsay命令后加上-f xxx 来改变卡通形象，这里的xxx参数是这些*.cow文件的文件名（从命令手册中可以了解到，这些文件作者称之为cowfile），作为参数时可以是完整的文件名也可以是不包含.cow的部分。怎么样，好玩吧，有没有觉得Linux的命令行似乎被赋予了各种生命力？哈哈哈哈哈～
...
按q键即可退出特效回到原先的提示符下，按b可以在下落的字符中随机的将一些字符变成粗体字，按B可以将所有下落字符都变成粗体字，按n可以关闭所有的粗体字特效，按0-9可以改变下落速度……
...
fortune能在命令行里随机显示一条谚语之类的英文句子，这倒是个不错的英语学习工具，随便输出几条看看效果吧：
...
oneko能召唤一只小猫咪，追着你的鼠标指针在屏幕上跑来跑去，这个必须要有图形化的支持，如果安装的是纯文本界面的Linux它是跑不起来的，一旦执行oneko，该进程默认会一直占用当前shell的前台直到我们按下ctrl+c才会退出来，效果在这里就不展示了，大家看看手册然后自己实验吧：
